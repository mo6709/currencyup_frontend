...
const auth = path => accounytInfo => {
	const { accountType } = accounytInfo;
	const account = !!accountType ? accountType : accounytInfo[Object.keys(accounytInfo)[0]].accountType;
	return fetch(baseURL + `${account}_${path}`, {
    method: 'POST',
    headers: headers,
